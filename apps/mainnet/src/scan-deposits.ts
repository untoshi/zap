...
async function listUtxo(base: string, headers: Record<string,string>, addr: string): Promise<Array<{ txid: string; vout: number; value: number }>> {
  try {
...
async function main() {
  ...
  for (const a of staticAddrs) {
    const utxos = await listUtxo(base, headers, a);
    if (utxos.length) {
  ...
  process.stdout.write(JSON.stringify({ items, info: { base, staticCount: staticAddrs.length, activeCount: activeAddrs.length } }));
}
